...
class Network_Job_ImportItems extends Omeka_Job_AbstractJob
{
    ...
    public function perform()
    {
        ...
        $_records  = $this->_db->getTable('NetworkRecord');
        $_exhibits = $this->_db->getTable('NetworkExhibit');
        ...
        while ($items = $_items->findBy($query, 10, $i)) {
            foreach ($items as $item) {
                ...
                if (!$record) {
                    $record = new NetworkRecord($exhibit, $item);
