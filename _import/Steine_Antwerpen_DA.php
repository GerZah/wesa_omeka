<?php
	...
	$csv=array();
	$file = fopen('Antwerpen_Rathaus_omeka_20161106.csv', 'r');
	...
	foreach($csv as $line) {
		...
		# 0. Don't import "RhANT", but calculate its item ID -- which is #???? --
		...
		# 6. Figure out the linked item ID from the stored item IDs array (starting with "RhANT" / ????)
		...
		if ($shortName == "RhANT") {
			# Step 0: Don't import "RhANT", but calculate its item ID -- which is #????
			$sql = "
				SELECT record_id FROM `$db->ElementTexts`
				WHERE text='Antwerpen, Rathaus' AND element_id = $titleElementID
			";
			$rhAntId = $db->fetchOne($sql);
			if ($rhAntId) { $alreadyCreated[$shortName] = $rhAntId; }
			...
			$line[$headers["Depth"]] = 0; // Q&D fix to compensate for the missing "Depth" column

			$hasMeasurements = (
				($line[$headers["Length"]] != "0,00")
				and ($line[$headers["Height"]] != "0,00")
				// and ($line[$headers["Depth"]] != "0,00")
			);
			...
			$link = $line[$headers["Link"]];
			// $link = ( $link != "Aedi_MRIS_MGi_RhLEI" ? $link : "" ); # +#+#+# DEBUG -- keine Verlinkung von hier
			if ($link) {
