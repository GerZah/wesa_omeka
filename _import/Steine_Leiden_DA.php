<?php
	...
	foreach($csv as $line) {
		...
		# 0. Don't import "RhLEI", but calculate its item ID -- which is #1299 --
		...
		# 6. Figure out the linked item ID from the stored item IDs array (starting with "RhLEI" / 1299)
		...
		if ($shortName == "RhLEI") {
			# Step 0: Don't import "RhLEI", but calculate its item ID -- which is #1299
			$sql = "
				SELECT record_id FROM `$db->ElementTexts`
				WHERE text='Leiden, Rathaus' AND element_id = $titleElementID
			";
			$rhLeiId = $db->fetchOne($sql);
			if ($rhLeiId) { $alreadyCreated[$shortName] = $rhLeiId; }
