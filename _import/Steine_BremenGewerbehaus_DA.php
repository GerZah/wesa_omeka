<?php
	...
	$csv=array();
	$file = fopen('Bremen_gewerbehaus_omeka_20161106.csv', 'r');
	...
	foreach($csv as $line) {
		...
		# 0. Don't import "Schuetting", but calculate its item ID -- which is #???? --
		...
		# 6. Figure out the linked item ID from the stored item IDs array (starting with "Schuetting" / ????)
		...
		if ($shortName == "BrGe") {
			...
			$sql = "
				...
				WHERE text='Bremen, Gewerbehaus' AND element_id = $titleElementID
			";
			$gewerbehausId = $db->fetchOne($sql);
			if ($gewerbehausId) { $alreadyCreated[$shortName] = $gewerbehausId; }
